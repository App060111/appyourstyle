...
function fallbackAnswer(query = "", links = []) {
  ...
  if (q.includes("hoka") && (q.includes("nike") || q.includes("44"))) return "Wenn Nike EU 44 passt, prüfe bei Hoka meist EU 44 2/3. Für lange Wege, breiteren Fuß oder mehr Komfort ist EU 45 oft die bessere Wahl.";
  if (q.includes("rockkonzert") || q.includes("konzert")) return "Für ein Rockkonzert empfehle ich einen robusten, bequemen Look: dunkle Jeans, bequeme Sneaker oder Boots, Shirt/Overshirt und leichte Jacke. Wichtig sind Beweglichkeit, sichere Taschen und unempfindliche Materialien.";
  if (q.includes("hochzeit") || q.includes("braut")) return "Für eine Hochzeit zuerst vorhandene Kleidung farblich ordnen. Wähle eine ruhige Grundfarbe und ergänze nur gezielt mit Tasche, Schuhen oder Schmuck. Ziel ist harmonisch, nicht komplett neu.";
  if (q.includes("london") || q.includes("reise") || q.includes("tage") || q.includes("kind")) return "Für Reise und Kind zählt Komfort: bequeme Schuhe, Layering, wetterfeste Jacke und wenige kombinierbare Teile. Plane lieber 2–3 flexible Outfits als zu viele Einzelteile.";
  if (links.length) return "Die eingegebenen Produktlinks wurden berücksichtigt. AppYourStyle bewertet sie als Outfit-Kombination nach Stil, Anlass, Farbe, Komfort und sinnvollen Ergänzungen. Ungültige oder leere Links werden ignoriert.";
...
export async function GET() {
  return Response.json({ success: true, result: "AppYourStyle API ist bereit.", visualPrompt: "API-Test erfolgreich.", outfitItems: [], products: [] });
}
export async function POST(req) {
  try {
    ...
    const fallback = { success: true, mode: "fallback", result: fallbackAnswer(query, links), visualPrompt: visualPromptFor(query, links), outfitItems: fallbackOutfitItems(query), products: fallbackProducts(query) };
    ...
    return Response.json({ success: true, mode: "openai", result: String(data?.choices?.[0]?.message?.content || fallback.result), visualPrompt: fallback.visualPrompt, outfitItems: fallback.outfitItems, products: fallback.products });
  } catch (error) {
    return Response.json({ success: true, mode: "fallback_exception", result: fallbackAnswer(""), visualPrompt: "Visualisierung wird vorbereitet.", outfitItems: fallbackOutfitItems(""), products: fallbackProducts(""), debug: error?.message || "unknown" });
  }
}
